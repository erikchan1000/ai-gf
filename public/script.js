...
async function getClientVersion() {
    try {
        ...
        $('#version_display').text("AI_GF v0.0.1");
        $('#version_display_welcome').text("AI_GF v0.0.1");
    } catch (err) {
...
async function Generate(type, { automatic_trigger, force_name2, quiet_prompt, quietToLoud, skipWIAN, force_chid, signal, quietImage, maxLoops } = {}, dryRun = false) {
    ...
    if (true === dryRun ||
        ...
        if (!dryRun && type !== 'quiet') {
            ...
            console.debug('Core chat', coreChat);
        ...
        if (main_api == 'koboldhorde' || main_api == 'kobold') {
            ...
            console.debug('Textgenwebui generation data');
            console.debug('Prompt: ' + finalPrompt);
        ...
        async function onSuccess(data) {
            ...
            if (!data.error) {
                ...
                console.debug('getMessage: ' + getMessage);
